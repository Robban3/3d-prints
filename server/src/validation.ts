...
export function parseCustomer(input: unknown): CustomerDetails {
  ...
  if (!EMAIL.test(email))
    errors["customer.email"] = "Ange en giltig e-postadress.";
...
export function parseQuoteRequest(input: unknown): CustomQuoteRequest {
  ...
  if (!materialById.has(material as never))
    errors.material = "Välj ett material.";
  if (!qualityById.has(quality as never))
    errors.quality = "Välj en utskriftskvalitet.";
  ...
  return {
    ...
    postProcessing:
      raw.postProcessing === true || raw.postProcessing === "true",
  };
}
